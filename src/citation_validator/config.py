...
class Config:
    ...
    def validate(self) -> bool:
        ...
        if not self.use_mock_llm and self.llm_provider == 'openai':
            if not self.openai_api_key:
                logging.warning("OpenAI API key not provided but required for non-mock mode")
                return False
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "llm": "Mock" if self.use_mock_llm else f"{self.llm_provider} ({self.openai_model})",
            "input_dir": self.input_dir,
            "output_dir": self.output_dir,
            "output_file": self.output_file,
            "log_level": self.log_level,
            "api_key_configured": bool(self.openai_api_key) if not self.use_mock_llm else "N/A"
        }
