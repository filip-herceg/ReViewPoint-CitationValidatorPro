...
class SourceContentSimulator:
    ...
    def simulate_source_content(self, citation: Dict[str, Any]) -> str:
        ...
        if any(keyword in title for keyword in ['chatbot', 'conversational', 'ai', 'artificial intelligence']):
            source_category = "ai_research"
        elif any(keyword in title for keyword in ['code', 'completion', 'development', 'programming']):
            source_category = "code_completion"
        elif any(keyword in title for keyword in ['privacy', 'security', 'data protection', 'enterprise']):
        ...
        content_index = hash(citation.get('doi', citation.get('title', ''))) % len(content_list)
...
class CitationValidator:
    ...
    def __init__(self, input_dir: str = "input", output_file: str = None):
        ...
        if not USE_MOCK_LLM and OPENAI_API_KEY:
            ...
            logger.warning("OpenAI API key not provided. Set OPENAI_API_KEY in .env file.")
    ...
    def load_footnotes(self) -> Dict[int, Dict[str, Any]]:
        ...
        if not self.footnotes_json_path.exists():
            raise FileNotFoundError(f"Footnotes file not found: {self.footnotes_json_path}")
    ...
    def create_llm_prompt(self, div_text: str, citation_metadata: Dict[str, Any], source_content: str = None) -> str:
...
Keep your response professional, concise but thorough (aim for 150-300 words)."""
    ...
    def get_mock_llm_response(self, prompt: str) -> str:
        """Generate a mock LLM response - only used if explicitly enabled."""
        logger.warning("Using mock LLM response. Set USE_MOCK_LLM=false in .env for real AI responses.")
        return "⚠️ **Mock Response**: This is a placeholder response. Configure your OpenAI API key in .env file and set USE_MOCK_LLM=false to get real AI evaluation."
    
    def get_openai_response(self, prompt: str) -> str:
        """Get actual response from OpenAI API."""
        if not OPENAI_API_KEY:
            logger.error("OpenAI API key not provided. Set OPENAI_API_KEY in .env file.")
        ...
        try:
            ...
            response = client.chat.completions.create(
                model=OPENAI_MODEL,
                messages=[
                    {"role": "system", "content": "You are an expert academic reviewer specializing in citation analysis. Provide thorough, professional evaluations of citation usage."},
    ...
    def process_divs(self, soup: BeautifulSoup, footnotes: Dict[int, Dict[str, Any]]) -> BeautifulSoup:
        ...
        for div in divs_with_ids:
            ...
            if USE_SIMPLIFIED_MODE:
                source_content = self.source_simulator.simulate_source_content(citation_metadata)
            ...
            processed_count += 1
            logger.info(f"Processed div {footnote_id} ({processed_count}/{len(divs_with_ids)})")
    ...
    def run_pipeline(self) -> None:
        ...
        logger.info(f"LLM: {'Mock' if USE_MOCK_LLM else f'{LLM_PROVIDER} ({OPENAI_MODEL})'}")
