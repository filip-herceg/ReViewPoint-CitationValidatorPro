...
Keep your response professional, concise but thorough (aim for 150-300 words)."""
    ...
    def get_mock_llm_response(self, prompt: str) -> str:
        ...
        is_ai_related = any(term in prompt_lower for term in ["ai", "künstliche intelligenz", "chatbot", "code completion"])
        is_privacy_related = any(term in prompt_lower for term in ["datenschutz", "privacy", "sicherheit", "security"])
        is_technical = any(term in prompt_lower for term in ["system", "modell", "implementierung", "plattform"])
